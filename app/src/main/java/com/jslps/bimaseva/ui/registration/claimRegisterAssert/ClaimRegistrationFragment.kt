package com.jslps.bimaseva.ui.registration.claimRegisterAssert
...
class ClaimRegistrationFragment : BaseFragment() {
    ...
    var spinnerPg: AutoCompleteTextView? = null
    ...
    override fun onResume() {
        ...
        mListener?.onFragmentUpdate(Constant.setTitle,
            HeaderData(false,"Asset Registration Of SHG  Member")
        )
    }
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val view = inflater.inflate(R.layout.fragment_claimregistration, container, false)
        ...
        spinnerSHG = view.findViewById<AutoCompleteTextView>(R.id.sppiner_shg)
        ...
        spinnerPg?.setOnItemClickListener { parent, _, position, id ->
            val selectedPoi = parent.adapter.getItem(position) as BlockMasterClass?
            shgCode = selectedPoi?.pgcode
            spinnerPg?.setText(selectedPoi?.pgnmae)
        ...
        radioGroup?.setOnCheckedChangeListener(
            RadioGroup.OnCheckedChangeListener { group, checkedId ->
                ...
                if (radiovalue.equals("SHG")) {
                    shgLayout?.visibility = View.VISIBLE
                    pgLayout?.visibility = View.GONE
                    instituionType = 1
                    spinnerSHG?.setText("")
                    getSHGDataList(villageCode.toString())
                } else {
                    shgLayout?.visibility = View.GONE
                    pgLayout?.visibility = View.VISIBLE
                    instituionType = 2
                    spinnerPg?.setText("")
                    val arrayListShg =
                        Select.from<Table7Db>(
                            Table7Db::class.java).where(
                            Condition.prop("villagecode").eq(villageCode))
                            .list() as ArrayList<Table7Db>
                    if (arrayListShg.isNotEmpty()) {
                        val adapter = AutocompleteAdapterPG(
                            activity!!, R.layout.spiner_row_new, arrayListShg, "PG"
                        )
                        spinnerPg?.setAdapter(adapter)
                        spinnerPg?.threshold = 1
                    }else {
                        Sneaker.with(activity!!) // Activity, Fragment or ViewGroup
                            .setTitle("No PG Available")
                            .sneakError()
                    }
   ...
   /* private fun getPGDataList(villgaeCode: String) {
        if (DialogUtil.isConnectionAvailable(activity)) {
            DialogUtil.displayProgress(activity!!)
            val gson = GsonBuilder().setLenient().create()
            val interceptor = HttpLoggingInterceptor()
            ...
            val changePhotoResponseModelCall =
                apiServices.fetchDistrictBlockClusterAndOtherGetList(*//*"3408013008003"*//*villgaeCode, "PGList", "", "")
            changePhotoResponseModelCall.enqueue(object : Callback<String> {
                override fun onResponse(
                    ...
                    if (mStudentObject1.master.isNotEmpty()) {
                        spinnerPg?.isEnabled = true
                        updateSppinerDistrict(mStudentObject1.master, "PG")
                    } else {
                        Sneaker.with(activity!!) // Activity, Fragment or ViewGroup
                            .setTitle("No PG Available")
                            .sneakError()
